...
app.use(
  cors({
    origin: process.env.AWS_LAMBDA_FUNCTION_NAME ? true : "http://localhost:3000", // Allow all origins in Lambda, localhost only for local dev
...
app.get('/health', async (req, res) => {
  try {
      ...
      const dbStatus = { state: stateMap[readyState] || 'unknown' };
      ...
      // Redis health
      const redisStatus = { state: 'unknown' };
      try {
